var deployJava = {
    ...
    writeAppletTag : function( attributes,
                               parameters ) {
        var s = '<'+'applet ';
    ...
    versionCheck : function( versionPattern ) {
        ...
        if( matchData != null ) {
            ...
            for( var i = 0; i < list.length; ++i ) {
                if( deployJava.compareVersionToPattern( list[i],
                                                        patternArray,
                                                        familyMatch ) ) {
                    return true;
                }
    ...
    compareVersionToPattern : function( version,
                                        patternArray,
                                        familyMatch ) {
        var regex = "^(\\d+)(?:\\.(\\d+)(?:\\.(\\d+)(?:_(\\d+))?)?)?$";
    ...
    testForMSVM : function( ) {
        ...
        if( typeof oClientCaps != 'undefined' ) {
            var v = oClientCaps.getComponentVersion( clsid,
                                                     "ComponentID" );
            if( ( v == '' ) || ( v == '5,0,5000,0' ) ) {
                return false;
            } else{
    ...
    testUsingMimeTypes : function( version ) {
        ...
        for( var i = 0; i < navigator.mimeTypes.length; ++i ) {
            ...
            if( m != null ) {
                if( deployJava.compareVersions(m[1],
                                               version) ) {
                    return true;
                }
            }
    ...
    testUsingPluginsArray : function( version ) {
        ...
        for( var i = 0; i < navigator.plugins.length; ++i ) {
            ...
            if( s.search( /^Java Switchable Plug-in (Cocoa)/ ) != -1 ) {
                if( deployJava.compareVersions( "1.5.0",
                                                version) ) {
                    return true;
                }
                ...
                if( platform.indexOf( 'win' ) != -1 ) {
                    if( deployJava.compareVersions( "1.5.0",
                                                    version )
                        || deployJava.compareVersions( "1.6.0",
                                                       version ) ) {
                        return true;
                    }
                }
            ...
            for( var i = 0; i < navigator.mimeTypes.length; i++ ) {
                if( navigator.mimeTypes[i].type == deployJava.mimeType ) {
                    if( navigator.mimeTypes[i].enabledPlugin ) {
                        var $embed = $( '<embed id="deployJavaPlugin"/>' ).attr( {
                                type : deployJava.mimeType,
                                hidden : true,
                            } );
                        $('body').append( $embed );
                        written = true;
