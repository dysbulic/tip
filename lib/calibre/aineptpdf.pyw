...
# aineptpdf.pyw and double-click on it to run it.
...
#   6.1 - removing small bug with V3 ebooks (anon)
#   6.2 - changed to adeptkey4.der format for 1.7.2 support (anon)
#   6.3 - backward compatibility for 1.7.1 and old adeptkey.der
"""
...
class DecryptionDialog(Tkinter.Frame):
    def __init__(self, root):
        ...
        self.status = Tkinter.Label(self, text='Automated Inept PDF V6.3 - Greetings to ILC!')
    ...
    def get_inpath(self):
        inpath = tkFileDialog.askdirectory(parent=None, title='Select encrypted PDF source directory')
    ...
    def decrypt(self):
        ...
        # get regular expression match for pdfs
        match = r'.pdf'
...
##                print keypath
##                print inpath+"\\"+fname
##                print outpath+"\\"+root+"_decrypted.pdf"
                argv = [sys.argv[0], keypath, inpath+fname, outpath+root+".jb.decrypted.pdf"]
...
def gui_main():
    ...
    if RSA is None:
        ...
        tkMessageBox.showerror(
            "Automated INEPT PDF Decrypter",
            "This script requires PyCrypto, which must be installed "
            "separately.  Read the top-of-script comment for details.")
        return 1
    root.title('Automated INEPT PDF Decrypter')
