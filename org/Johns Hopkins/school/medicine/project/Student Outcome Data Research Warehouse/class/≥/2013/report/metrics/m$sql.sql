...
;WITH Competencies ( id, child_id, depth, path ) AS
( SELECT tree.competency_id, tree.child_id, 1, '|' + CAST(tree.competency_id AS VARCHAR(MAX)) + '|'
  ...
  UNION ALL
  SELECT tree.competency_id, tree.child_id, depth + 1, containing_tree.path  + '|' + CAST(tree.competency_id AS VARCHAR(MAX)) + '|'
    FROM rpt_competency_ge2013_tree_map tree
   INNER JOIN Competencies containing_tree -- recursive join
      ON containing_tree.child_id = tree.competency_id
   WHERE containing_tree.path NOT LIKE '%|' + CAST(tree.competency_id AS VARCHAR(MAX)) + '|%'
)
SELECT DISTINCT id, child_id, depth, path
FROM    Competencies
ORDER BY depth
