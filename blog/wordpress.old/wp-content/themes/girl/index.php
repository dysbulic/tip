...
<div <?php post_class(); ?>>
<div class="heading">{<?php the_time(get_option('date_format')) ?>} &nbsp;
	<span id="post-<?php the_ID(); ?>" class="entry-title">
	<?php if ( ! is_single() ) : ?>
		<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php echo esc_attr( sprintf( __( 'Permanent Link to %s', 'girl' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a>
	<?php else : ?>
		<?php the_title(); ?>
	<?php endif; ?>
	</span>
