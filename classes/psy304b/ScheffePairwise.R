#  Based on src/library/stats/R/TukeyHSD.R
...
###
###               Pairwise contrasts using Scheffe error correction
###

ScheffePairwise <-
    function(x, which, ordered = FALSE, conf.level = 0.95, ...)
    UseMethod("ScheffePairwise")
             ...
             conf.level = 0.95, ...)
{
    ...
    for (nm in names(tabs)) {
        ...
        width <- qf(conf.level, length(means), x$df.residual) *
            sqrt((MSE/2) * outer(1/n, 1/n, "+"))[keep]
    ...
    class(out) <- c("multicomp", "ScheffePairwise")
...
print.ScheffePairwise <- function(x, digits=getOption("digits"), ...)
{
    cat("  Scheffe multiple comparisons of means\n")
...
plot.ScheffePairwise <- function (x, ...)
{
